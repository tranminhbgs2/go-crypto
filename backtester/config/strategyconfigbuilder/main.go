...
func parseAPI(reader *bufio.Reader, cfg *config.Config) error {
	...
	defaultEnd := time.Now()
	fmt.Printf("What is the start date? Leave blank for \"%v\"\n", defaultStart.Format(time.RFC3339))
	startDate = quickParse(reader)
	if startDate != "" {
		cfg.DataSettings.APIData.StartDate, err = time.Parse(time.RFC3339, startDate)
		if err != nil {
			return err
		}
	} else {
	...
	fmt.Printf("What is the end date? Leave blank for \"%v\"\n", defaultEnd.Format(time.RFC3339))
	endDate = quickParse(reader)
	if endDate != "" {
		cfg.DataSettings.APIData.EndDate, err = time.Parse(time.RFC3339, endDate)
		if err != nil {
			return err
		}
	} else {
...
func parseDatabase(reader *bufio.Reader, cfg *config.Config) error {
	...
	startDate := quickParse(reader)
	if startDate != "" {
		cfg.DataSettings.DatabaseData.StartDate, err = time.Parse(time.RFC3339, startDate)
		if err != nil {
			return err
		}
	} else {
	...
	if endDate := quickParse(reader); endDate != "" {
		cfg.DataSettings.DatabaseData.EndDate, err = time.Parse(time.RFC3339, endDate)
		if err != nil {
			return err
		}
	} else {
