...
var executeStrategyFromFileCommand = &cli.Command{
	...
	Flags: []cli.Flag{
		...
		&cli.StringFlag{
			...
			Aliases: []string{"s"},
			Usage:   fmt.Sprintf("override the strategy file's start time using your local time. eg '%v'", time.Now().Truncate(time.Hour).AddDate(0, -1, 0).Format(time.RFC3339)),
		},
		&cli.StringFlag{
			Name:    "endtimeoverride",
			Aliases: []string{"e"},
			Usage:   fmt.Sprintf("override the strategy file's end time using your local time. eg '%v'", time.Now().Truncate(time.Hour).Format(time.RFC3339)),
		},
		&cli.Uint64Flag{
...
func executeStrategyFromFile(c *cli.Context) error {
	...
	if startTimeOverride != "" {
		s, err = time.ParseInLocation(time.RFC3339, startTimeOverride, time.Local)
		if err != nil {
			return fmt.Errorf("invalid time format for start: %v", err)
		}
	}
	if endTimeOverride != "" {
		e, err = time.ParseInLocation(time.RFC3339, endTimeOverride, time.Local)
		if err != nil {
