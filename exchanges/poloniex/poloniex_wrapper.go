...
allTrades:
	for {
		...
		for i := range tradeData {
			var tt time.Time
			tt, err = time.Parse(time.RFC3339, tradeData[i].Date)
			if err != nil {
				return nil, err
			}
...
func (p *Poloniex) GetOrderInfo(ctx context.Context, orderID string, pair currency.Pair, _ asset.Item) (*order.Detail, error) {
	...
	for i := range trades {
		...
		tradeHistory.Timestamp, err = time.Parse(time.RFC3339, trades[i].Date)
		if err != nil {
			return nil, err
		}
	...
	orderInfo.Date, err = time.Parse(time.RFC3339, resp.Date)
	if err != nil {
		return nil, err
	}
...
func (p *Poloniex) GetActiveOrders(ctx context.Context, req *order.MultiOrderRequest) (order.FilteredOrders, error) {
	...
	for key := range resp.Data {
		...
		for i := range resp.Data[key] {
			...
			orderDate, err = time.Parse(time.RFC3339, resp.Data[key][i].Date)
			if err != nil {
				log.Errorf(log.ExchangeSys,
...
func (p *Poloniex) GetOrderHistory(ctx context.Context, req *order.MultiOrderRequest) (order.FilteredOrders, error) {
	...
	for key := range resp.Data {
		...
		for i := range resp.Data[key] {
			...
			orderDate, err := time.Parse(time.RFC3339,
				resp.Data[key][i].Date)
			if err != nil {
				log.Errorf(log.ExchangeSys,
					"Exchange %v Func %v Order %v Could not parse date to unix with value of %v",
