...

		{
			...
			Flags: []cli.Flag{
				...
				&cli.StringFlag{
					...
					Value:       time.Now().AddDate(-1, 0, 0).Truncate(time.Hour).Format(time.RFC3339),
		...
		},
		{
			...
			Flags: []cli.Flag{
				...
				&cli.StringFlag{
					...
					Value:       time.Now().AddDate(0, -1, 0).Truncate(time.Hour).Format(time.RFC3339),
					Destination: &startTime,
				},
				&cli.StringFlag{
					Name:        "end",
					...
					Value:       time.Now().Truncate(time.Hour).Format(time.RFC3339),
					Destination: &endTime,
				},
				&cli.StringFlag{
...
func getFuturesPositions(c *cli.Context) error {
	...
	s, err = time.ParseInLocation(time.RFC3339, startTime, time.Local)
	if err != nil {
		return fmt.Errorf("invalid time format for start: %v", err)
	}
	e, err = time.ParseInLocation(time.RFC3339, endTime, time.Local)
	if err != nil {
